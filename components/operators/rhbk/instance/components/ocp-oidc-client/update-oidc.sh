...
KEYCLOAK_URL="https://composer-ai-rhbk-service.composer-ai-rhbk.svc.cluster.local:8443"

echo "Connecting to Keycloak at "$KEYCLOAK_URL
echo "Verifying Keycloak is available"

until $(curl -k -s -o /dev/null "$KEYCLOAK_URL")
do
...
# curl -k -s "$KEYCLOAK_URL/admin/realms/openshift-ai/identity-provider/instances/openshift-v4" \
...
# curl -k -s -X PUT "$KEYCLOAK_URL/admin/realms/openshift-ai/identity-provider/instances/openshift-v4" \
# --header 'Authorization: Bearer '$access_token \
# --header 'Content-Type: application/json' \
...
# curl -k -s "$KEYCLOAK_URL/admin/realms/openshift-ai/clients" \
...
# curl -k -s -X PUT "$KEYCLOAK_URL/admin/realms/openshift-ai/clients/"$CLIENT_ID \
# --header 'Authorization: Bearer '$access_token \
# --header 'Content-Type: application/json' \
...
oc patch serviceaccount openshift-oidc -p '{"metadata":{"annotations": {"serviceaccounts.openshift.io/oauth-redirecturi.first": "https://keycloak-'$NAMESPACE'.'$APP_URL'/realms/openshift-ai/broker/openshift-v4/endpoint"}}}'
